            ...
            for (int il = 1; il <= count; il++) {
                ...
                if (!element.isSelected()) { // if unchecked is = false
                    try {
                        ...
                        MyDriver.errMessage("Strona rejestracji: checkbox " + text + " nie jest klikalny.");
                    }
                } else {
                    MyDriver.errMessage("Strona rejestracji: checkbox " + text + " jest już zaznaczony, czego nie może być.");
                }
            }
        } else {
            MyDriver.errMessage("Strona rejestracji nie zawiera checkbox`y ulubionych kategorii.");
        }
    }
